...
/**
 * <p>This class is a JavaFileManager to store compiled byte code in a memory map.</p>
 * 
 * <p>A JavaFileManager determines where to create new JavaFileObjects. In this context, 
 * <em>file</em> means an abstraction of regular files and other sources of data. For 
 * example, a file object can be used to represent regular files, memory cache, or data 
 * in databases.</p>
 * 
 ...
 */
public final class MemoryJavaFileManager<FM extends JavaFileManager>
	extends ForwardingJavaFileManager<FM>
{
     ...
     * 
     * @param fileManager A delegate to this file manager
     */
	...
	/**
	 * A subclass of JavaFileObject used to represent Java source coming from a string.
	 * 
	...
	/**
	 * A subclass of JavaFileObject used to output a Java target to a local class map.
	 */
	private class TargetJavaFileObject
		extends SimpleJavaFileObject
	{
     ...
     * given location.
     * 
     * @param location a location to search for file objects.
     * @param className the name of a class.
     * @param kind the kind of file, must be one of {@link JavaFileObject.Kind#SOURCE SOURCE} or {@link JavaFileObject.Kind#CLASS CLASS}
	 ...
	 * 
	 * @param className The name to identify the class.
	 * @param bytecode The class byte code.
	 * 
	 * @return A new JavaFileObject containing the bytcode and identified by the class name.
	 */
		...
		else
		{
			try
			{
				...
				if (name.endsWith(JAVA_SOURCE_EXTENSION))
					newUri.replace(newUri.length() - JAVA_SOURCE_EXTENSION.length(), 
						newUri.length(), JAVA_SOURCE_EXTENSION);
